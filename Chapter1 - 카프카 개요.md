...
v0.8: 리플리케이션 기능 추가
v0.8.2: 스키마 레지스트리 공개 --> 고급 인력이 데이터를 분석하거나, 비정형 데이터를 파싱하는데 많은 시간을 소모하는 것을 해결 가능하게 해주었다.
v0.9: 카프카 커넥트 공개 --> 카프카와 연동 가능한 DB와 프로토콜이 늘어나면서 발생한 유지보수의 어려움을 해결하기 위해 등장하였고, 별도의 코드 작성 없이 카프카와 연동할 수 있게 되었다.
v0.10: 카프카 스트림즈 공개 --> 가벼운 클라이언트로 실시간 분석과 처리가 가능해졌다.
